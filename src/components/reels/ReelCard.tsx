...
import { PreviewRenderer } from '@/components/media/PreviewRenderer';
    ...
    <>
      <div ref={containerRef} className={cn('relative h-full w-full bg-black', className)}>
        ...
        {isVideo && !isLocked && primaryMedia?.id ? (
          ...
          // Full-screen image fallback (signed URL via MediaRenderer)
          <MediaRenderer
            media={post.media}
            variant="reels"
            isLocked={false}
        ...
        {/* Locked advanced preview (blur/partial blur/crop/teaser + stickers + paywall CTA) */}
        {isLocked && (
          <PreviewRenderer
            media={post.media}
            variant="reels"
