...
export function SubscriptionSettingsContainer() {
  ...
  useEffect(() => {
    const fetchPricing = async () => {
      try {
        const data = await apiClient.creator.getProfile();
        setPricing({
          subscriptionPrice: Number(data.subscriptionPrice) || 9.99,
          subscriptionPrice3m: data.subscriptionPrice3m ? Number(data.subscriptionPrice3m) : null,
          subscriptionPrice6m: data.subscriptionPrice6m ? Number(data.subscriptionPrice6m) : null,
          subscriptionPrice12m: data.subscriptionPrice12m
            ? Number(data.subscriptionPrice12m)
            : null,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await apiClient.creator.updateSubscription(pricing);
      setSuccess(true);
